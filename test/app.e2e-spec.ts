...
describe('AppController (e2e)', () => {
  ...
  beforeEach(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [MongooseModule.forRoot('mongodb://localhost', { dbName: 'test' }), AppModule, UserModule],
    })
  ...
  it('/user (POST)', () => {
    ...
    return request(app.getHttpServer())
      ...
      .expect(({ body }) => {
        ...
        expect(body.message).toContain('User record has been created successfully!');
      });
  });
